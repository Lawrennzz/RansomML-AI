#!/usr/bin/env python3
"""
Test script for Enhanced Ransomware Detection System
"""
...
def test_enhanced_system():
    ...
    try:
        ...
        if data['success']:
            ...
            print(f"❌ Failed to get stats: {data['message']}")
    except Exception as e:
        print(f"❌ Error getting stats: {e}")
    
    # Test 3: Test benign sample
    print("\n3. Testing benign sample detection...")
    benign_sample = {
    ...
    try:
        ...
        print(f"❌ Error predicting benign sample: {e}")
    
    # Test 4: Test ransomware sample
    print("\n4. Testing ransomware sample detection...")
    ransomware_sample = {
    ...
    try:
        ...
        if data['success']:
            result = data['result']
            print("✅ Ransomware sample analyzed!")
            print(f"   Prediction: {'Ransomware' if result['prediction'] else 'Benign'}")
            print(f"   Confidence: {result['confidence']:.4f}")
            print(f"   Risk Level: {result['risk_level']}")
        else:
        ...
        print(f"❌ Error predicting ransomware sample: {e}")
    
    # Test 5: Test detection history
    print("\n5. Testing detection history...")
    try:
        response = requests.get(f"{base_url}/api/detection-history")
        data = response.json()
        
        if data['success']:
            history = data['history']
            print(f"✅ Detection history retrieved! ({len(history)} entries)")
            if history:
                latest = history[-1]
                print(f"   Latest detection: {latest['result']['timestamp']}")
        else:
            print(f"❌ Failed to get history: {data['message']}")
    except Exception as e:
        print(f"❌ Error getting history: {e}")
    
    # Test 6: Test feature info
    ...
    try:
        ...
        if data['success']:
            ...
            print(f"✅ Feature information retrieved! ({len(features)} features)")
            print("   Top features:")
            for i, (feature, description) in enumerate(list(features.items())[:3]):
                print(f"     {i+1}. {feature}: {description}")
        else:
            print(f"❌ Failed to get feature info: {data['message']}")
    except Exception as e:
